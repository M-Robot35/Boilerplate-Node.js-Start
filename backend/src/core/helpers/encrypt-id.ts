...
function encrypt(text:any) {
  ...
  logSystem.success('encrypt success')
...
export function decrypt(encrypted:string) {
  ...
  try{
    ...
    logSystem.success('decrypted success')
    return decrypted;

  }catch(error){
    logSystem.error(`decrypt --- ${JSON.stringify(error)}`)
  }
}
...
export function getOrGenerateHash(id:any) {
    ...
    if (!fs.existsSync(path.dirname(pathLog))) {
        FileMestre.writeJson(pathLog, {})
        logSystem.success('criado um novo diretorio') 
    }
